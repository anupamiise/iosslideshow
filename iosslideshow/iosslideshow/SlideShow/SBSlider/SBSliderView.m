...
- (void)createSliderWithImagesRemote:(NSArray *)images WithAutoScroll:(BOOL)isAutoScrollEnabled inView:(UIView *)parentView
{
    //    self.backgroundColor = [UIColor blackColor];
    ...
    for (int x = 0; x < 3; x++) {
        ...
        for (int i=0; i < [imagesArray count]; i++)
        {
            ...
            AsyncImageView *imageV = [[AsyncImageView alloc] init];
            CGRect frameRect;
            frameRect.origin.y = 0.0f;
            frameRect.size.width = [UIScreen mainScreen].bounds.size.width;
            frameRect.size.height = _sliderMainScroller.frame.size.height;
...
//            imageV.image = nil;
            imageV.imageURL = [NSURL URLWithString:(NSString *)[imagesArray objectAtIndex:i]];
            imageV.crossfadeDuration = 0.1;
            [_sliderMainScroller addSubview:imageV];
            imageV.clipsToBounds = YES;
            imageV.userInteractionEnabled = YES;
